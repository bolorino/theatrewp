		...
		<div id="content" role="main">
			...
			<div class="entry-content">
				...
				if ( ! empty( $calendar ) ) { ?>
					<p>
						<?php
						printf( _n( 'One performance', '%s Total performances', $total_performances, 'theatre-wp' ), $total_performances );

						if ( $dates_sent OR isset( $_SESSION['month'] ) && $_SESSION['month']  > 1 ) { ?>
							<a href="<?php echo $action_form; ?>"><?php echo __( 'View upcoming performances', 'theatre-wp' ); ?></a>
					<?php }	?>
					</p>
					<?php
							...
							<div class="performance">
								...
								if ( $performance_custom['date_last'] ) {
									...
									if ( $first_time == $last_time) {
										$performance_time = sprintf( _x( 'From %s to %s (%s)', 'From day one to day two at (hour)', 'theatre-wp' ), $performance_first_date, $performance_last_date, $first_time );
									} else {
										$performance_time = sprintf( _x( 'From %s (%s) to %s (%s)', 'From day one at (hour) to day two at (hour)', 'theatre-wp' ), $performance_first_date, $first_time, $performance_last_date, $last_time );
									}
								} else {
			...
			<?php
      if ( function_exists('performances_pagination') ) { 
        performances_pagination( $calendar_filter_params );
      }
      ?>
